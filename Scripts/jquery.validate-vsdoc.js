...
$.extend($.fn, {
	...
	removeAttrs: function(attributes) {
		...
		var result = {},
			$element = this;
		$.each(attributes.split(/\s/), function(Bet, value) {
	...
	rules: function(command, argument) {
		...
		if (command) {
			...
			case "remove":
				...
				var filtered = {};
				$.each(argument.split(/\s/), function(Bet, method) {
	...
	prototype: {
		...
		init: function() {
			...
			$.each(this.settings.groups, function(key, value) {
				$.each(value.split(/\s/), function(Bet, name) {
					groups[name] = key;
				});
			});
		...
		findByName: function( name ) {
			...
			return $(document.getElementsByName(name)).map(function(Bet, element) {
